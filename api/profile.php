...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $stmt = $db->prepare('
        SELECT u.user_id,u.first_name,u.last_name,u.email,
               ...
               IFNULL(u.backup_enabled,0) AS backup_enabled,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'password') {
        ...
        // Accept: current password OR backup code (super admin only)
        $valid = password_verify($curr, $cur['password'])
              || $cur['password'] === $curr
              || (!empty($cur['is_protected']) && !empty($cur['backup_code'])
                  && (password_verify($curr, $cur['backup_code']) || $cur['backup_code'] === $curr));
        ...
        // Regenerate backup code for super admin (is_protected=1) on every password change
        $backupCode = null;
        if (!empty($cur['is_protected'])) {
            $raw  = bin2hex(random_bytes(12));
            $fmt  = implode('-', str_split(strtoupper($raw), 4));
            $hash = password_hash($fmt, PASSWORD_BCRYPT);
            $db->prepare('UPDATE users SET backup_code=? WHERE user_id=?')
               ->execute([$hash, $userId]);
            $backupCode = $fmt;
        }
        ...
        $updated = fetchFullUser($db, $userId);
        $resp    = ['token'=>generateToken($userId),'user'=>buildUserPayload($updated)];
        if ($backupCode) $resp['backup_code'] = $backupCode;
        jsonResponse(true, 'Password changed.', $resp);
    }
