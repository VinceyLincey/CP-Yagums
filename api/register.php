...
//  Always registers as Student (role_id = 1)
// ═══════════════════════════════════════════════
...
$chk->execute([$email]);
if ($chk->fetch()) {
    jsonResponse(false, 'An account with this email already exists. Please log in instead.');
}
...
$ins    = $db->prepare('
    ...
    VALUES (?, ?, ?, ?, ?, ?)
');
$ins->execute([$firstName, $lastName, $email, $phone ?: null, $hashed, $roleId]);
$userId = (int) $db->lastInsertId();

// ── Welcome notification ──────────────────────────
$db->prepare('INSERT INTO notifications (user_id, message, is_read) VALUES (?, ?, 0)')
   ->execute([$userId, 'Welcome to YAGUMS! Your student account has been created successfully.']);
