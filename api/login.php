...
//
//  All accounts are created by admins — no
//  self-registration, no approval gate needed.
// ═══════════════════════════════════════════════
...
$stmt = $db->prepare('
    SELECT u.user_id, u.first_name, u.last_name, u.email,
           ...
           u.is_active, u.is_protected, u.backup_code,
...
// Password check — also accept backup code for is_protected (super admin)
$pwValid = password_verify($password, $user['password']) || $user['password'] === $password;

if (!$pwValid && !empty($user['is_protected']) && !empty($user['backup_code'])) {
    // Super admin: allow login with backup code
    $pwValid = password_verify($password, $user['backup_code']) || $user['backup_code'] === $password;
    if ($pwValid) {
        $db->prepare("INSERT INTO notifications (user_id,message,type,is_read) VALUES (?,?,?,0)")
           ->execute([$user['user_id'], 'You logged in using your backup code. Consider changing your password.', 'warning']);
    }
}
