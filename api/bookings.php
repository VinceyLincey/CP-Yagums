...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cancel') {
        if (!$bookingId) jsonResponse(false, 'booking_id required.');
        // Fetch booking — must belong to this user
        $stmt = $db->prepare('SELECT b.user_id, b.status_id, f.facility_name, b.booking_date FROM bookings b JOIN facilities f ON b.facility_id=f.facility_id WHERE b.booking_id=? LIMIT 1');
        $stmt->execute([$bookingId]);
        $bk = $stmt->fetch();
        if (!$bk) jsonResponse(false, 'Booking not found.');
        if ((int)$bk['user_id'] !== $userId) jsonResponse(false, 'You can only cancel your own bookings.');
        if (in_array((int)$bk['status_id'], [3,4])) jsonResponse(false, 'This booking is already '.((int)$bk['status_id']===4?'cancelled':'rejected').'.');
        $db->prepare('UPDATE bookings SET status_id=4, updated_at=NOW() WHERE booking_id=?')->execute([$bookingId]);
        $db->prepare("INSERT INTO notifications (user_id,message,type,is_read) VALUES (?,?,?,0)")
           ->execute([$userId, "Your booking for {$bk['facility_name']} on {$bk['booking_date']} has been cancelled.", 'info']);
        jsonResponse(true, 'Booking cancelled.');
    }

    jsonResponse(false, 'Unknown action. Use "cancel".');
}
